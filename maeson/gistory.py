    ...
    def update_scene(self):
        ...
        for layer_def in scene.layers:
            if layer_def["type"] == "geojson":
                ...
                try:
                    bounds = layer_def["bounds"]  # Must be [[south, west], [north, east]]
                    url = layer_def["url"]
                    layer = ImageOverlay(
                        url=url,
                        bounds=bounds,
...
class SceneBuilder:
    def __init__(self, maeson_map):
        ...
        self.order_input = widgets.IntText(
            description="Order",
            value=1,
            min=1
        )
        ...
        self.sort_chrono = widgets.Checkbox(
            value=False,
            description="Sort Chronologically",
            indent=False
        )

        self.layer_type = widgets.Dropdown(
            options=['tile', 'geojson', 'image', 'raster', 'wms', 'video', 'earthengine'],
    ...
    def detect_layer_type(path):
        ...
        if path.startswith("projects/") or path.count("/") >= 2 and not path.startswith("http"):
            return "earthengine"
